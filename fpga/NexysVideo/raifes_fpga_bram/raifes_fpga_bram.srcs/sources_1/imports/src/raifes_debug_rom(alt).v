...
assign	dm_debugrom[0] = `XPR_LEN'h15C02023;	// 0001 0101 1100 0000 0010 0000 0010 0011, 14802023, sw s28, 0x140(x0) -- push t3 to debug mem space 0x140
...
assign  dm_debugrom[10] = `XPR_LEN'h14002E03;	// 0001 0100 0000 0000 0010 1110 0000 0011, 14002403, lw s28, 0x140(x0) -- pop t3 from "stack"
...
assign	dm_debugrom[81] = {status[`XPR_LEN-1:1],resume_req&~status[0]};
	...
	if(reset) begin	
		...
		status <= `XPR_LEN'h0;
			...
			if(rom_writeb && (rom_addrb == `ADDR_HART0_STATUS))
				status[1:0] <= (rom_wdatab == 0) ? {status[1],1'b1} : {rom_wdatab[1],rom_wdatab[0]};	
