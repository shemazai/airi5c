...
input  wire [31:0]  cpr_rs1          , // Source register 1
...
wire [ 3:0] cop_mem_writea = cop_mem_ben &&{4{cop_mem_wen}};
wire mem_is_load;

wire is_mem  = mem_ivalid;

wire is_lw   = is_mem && id_subclass[RAIFES_COP_SCLASS_LD_W];
wire is_lh   = is_mem && id_subclass[RAIFES_COP_SCLASS_LH_CR];
wire is_lb   = is_mem && id_subclass[RAIFES_COP_SCLASS_LB_CR];
wire is_sw   = is_mem && id_subclass[RAIFES_COP_SCLASS_ST_W];
wire is_sh   = is_mem && id_subclass[RAIFES_COP_SCLASS_ST_H];
wire is_sb   = is_mem && id_subclass[RAIFES_COP_SCLASS_ST_B];
wire is_ldr_w= is_mem && id_subclass[RAIFES_COP_SCLASS_LDR_W];
wire is_ldr_h= is_mem && id_subclass[RAIFES_COP_SCLASS_LDR_H];
wire is_ldr_b= is_mem && id_subclass[RAIFES_COP_SCLASS_LDR_B];
wire is_str_w= is_mem && id_subclass[RAIFES_COP_SCLASS_STR_W];
wire is_str_h= is_mem && id_subclass[RAIFES_COP_SCLASS_STR_H];
wire is_str_b= is_mem && id_subclass[RAIFES_COP_SCLASS_STR_B];
...
wire halfword_op     = halfword_op_nsg;
wire byte_op         = byte_op_nsg;
...
assign mem_is_store   = is_sw    || is_sh || is_sb ||
                        is_str_w || is_str_h || is_str_b ;
...
wire wb_hw_hi = (halfword_ld_nsg &&  id_wb_h) ;
wire wb_hw_lo = (halfword_ld_nsg && !id_wb_h) ;

wire wb_b_3   = (byte_ld_nsg &&  id_wb_h &&  id_wb_b) ;
wire wb_b_2   = (byte_ld_nsg &&  id_wb_h && !id_wb_b) ;
wire wb_b_1   = (byte_ld_nsg && !id_wb_h &&  id_wb_b) ;
wire wb_b_0   = (byte_ld_nsg && !id_wb_h && !id_wb_b) ;
...
assign mem_cpr_rd_ben[3] = word_ld || wb_hw_hi || wb_b_3 || clear_hb || clear_hh;
assign mem_cpr_rd_ben[2] = word_ld || wb_hw_hi || wb_b_2 || clear_hb || clear_hh;
assign mem_cpr_rd_ben[1] = word_ld || wb_hw_lo || wb_b_1 || clear_hb;
assign mem_cpr_rd_ben[0] = word_ld || wb_hw_lo || wb_b_0;
...
always @(*) begin: p_n_mem_fsm
    case(mem_fsm)
        ...
        FSM_SG_B_1:
            n_mem_fsm = FSM_SG_B_2 ;
        FSM_SG_B_2:
            n_mem_fsm = FSM_SG_B_3 ;
        FSM_SG_B_3:
            n_mem_fsm = FSM_IDLE ;
        default:
            n_mem_fsm = FSM_IDLE;
...
always @(posedge g_clk) begin
  ...
  blk_mem_gen_1 Block_RAM2 (
  .clka(g_clk),    // input wire clka
  .wea(cop_mem_writea),      // input wire [3 : 0] wea
  .addra(cop_mem_addr),  // input wire [31 : 0] addra
  .dina(cop_mem_wdata),    // input wire [31 : 0] dina
  .douta(cop_mem_rdata)  // output wire [31 : 0] douta
);
